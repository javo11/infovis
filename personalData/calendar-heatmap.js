...
function calendarHeatmap() {
  ...
  function chart() {
      ...
      if (chart.tooltipEnabled()) {
        dayRects.on('mouseover', function (d, i) {
          tooltip = d3.select('body')
            ...
            .style('left', function () { return 150 + (Math.floor(i / 7) * SQUARE_LENGTH) + "px"; })
            .style('top', function () { return 330 + (d.getDay() * (SQUARE_LENGTH + SQUARE_PADDING) + MONTH_LABEL_PADDING * 3) + "px"; });
        })
    ...
    function tooltipHTMLForDate(d) {
      ...
      var count = countForDate(d);
      return '<span><strong>' + (count ? Math.round(count*100)/100 : '0') + tooltipUnit + '</strong> on ' + dateStr + '</span>';
    }
